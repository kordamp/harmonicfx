...
public class SubmitButton extends Region {
    ...
    private        Ikon                   successIkon      = Ikonli.NONE;
    private        Ikon                   failIkon         = Ikonli.NONE;
        ...
        status           = new ObjectPropertyBase<Status>(Status.NONE) {
            @Override protected void invalidated() {
                switch(get()) {
                    ...
                    case SUCCESS    : icon.setIconCode(successIkon); break;
                    case FAIL       : icon.setIconCode(failIkon); formerColor = getColor(); color.set(Color.CRIMSON); animateFromProgressFail(); break;
    ...
    public Ikon getSuccessIkon() { return successIkon; }
    public Ikon getFailIkon() { return failIkon; }
    public void setSuccessIkon(Ikon ikon) { successIkon = ikon; }
    public void setFailIkon(Ikon ikon) { failIkon = ikon; }

    private void animateUpToProgress() {
        if (status.get() == Status.IN_PROGRESS) return;
