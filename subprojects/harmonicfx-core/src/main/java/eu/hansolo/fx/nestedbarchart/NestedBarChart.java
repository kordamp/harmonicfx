...
import eu.hansolo.fx.nestedbarchart.event.SelectionEventListener;
import eu.hansolo.fx.nestedbarchart.series.Series;
import eu.hansolo.fx.nestedbarchart.tools.Helper;
import eu.hansolo.fx.nestedbarchart.tools.InfoPopup;
import eu.hansolo.fx.nestedbarchart.tools.Order;
...
public class NestedBarChart extends Region {
    ...
    private              Pane                                         pane;
    private              ObservableList<Series<Item>>                 series;
    ...
    public NestedBarChart(final Series<Item>... SERIES) {
        this(Arrays.asList(SERIES));
    }
    public NestedBarChart(final List<Series<Item>> SERIES) {
        getStylesheets().add(NestedBarChart.class.getResource("nested-bar-chart.css").toExternalForm());
    ...
    private void registerListeners() {
        ...
        setOnSelectionEvent(e -> {
            Series<Item> series = e.getSeries();
            Item         item   = e.getItem();
    ...
    public List<Series<Item>> getSeries() { return series; }
    public void setSeries(final Series<Item>... SERIES) {
        setSeries(Arrays.asList(SERIES));
    }
    public void setSeries(final List<Series<Item>> SERIES) {
        series.clear();
        SERIES.forEach(item -> series.add(item));
        redraw();
    }
    public void addSeries(final Series<Item> SERIES) {
    ...
    public void addSeries(final List<Series<Item>> SERIES) {
        SERIES.forEach(item -> addSeries(item));
    }
    public void removeSeries(final Series<Item> SERIES) {
        if (series.contains(SERIES)) {
    ...
    public void removeSeries(final List<Series<Item>> SERIES) {
    ...
    public void checkForClick(final MouseEvent EVT) {
        ...
        for (int i = 0 ; i < noOfBars  ;i++) {
            maxSum = Math.max(maxSum, series.get(i).getItems().stream().mapToDouble(Item::getValue).sum());
        }
        double stepY = height / maxSum;
        Series<Item> selectedSeries = null;
        for (int i = 0 ; i < noOfBars ; i++) {
            Series<Item> s = series.get(i);
            int noOfItems        = s.getNoOfItems();
            double sumOfItems    = s.getItems().stream().mapToDouble(Item::getValue).sum();
    ...
    private void sortItems(final List<Item> ITEMS, final Order ORDER) {
